...
- `resources/views/pages/⚡plans/plans.test.php` — 49 co-located Livewire tests (115 assertions): full CRUD with validation, localized error alerts, rate limiting, toast + close-modal success contracts, cancel-path state resets, complete/reopen flows, fa and en default month-boundary filters (via `travelTo`, asserting exact `Jalali::monthBounds` values), plans outside the filter window hidden, search filtering, no-results vs empty distinction, all five sort orders (with deterministic `created_at` seeding), pagination at 3 cards per page (island HTML contains exactly 3 `wire:key="plan-"` occurrences plus pager controls), Jalali card-date rendering, and the description fallback.
...
**Acceptance Result:** UC-07 is accepted. Authenticated users can create plans (with duplicate detection and rate limiting), edit plans from a modal (with duplicate detection and rate limiting), delete plans (blocked if tasks exist), browse plans filtered by a locale-aware default month window (Jalali in `fa`, Gregorian in `en`) with search, status filter, five sort orders, and pagination (3 per page), see per-plan progress bars and day counts, and complete/reopen plans. The use case is covered by 79 passing tests (49 Livewire co-located [115 assertions] + 8 create + 7 edit + 4 delete + 5 complete + 4 reopen action + 2 access).
