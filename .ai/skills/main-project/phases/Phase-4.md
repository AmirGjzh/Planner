...
- `resources/views/pages/auth/⚡login/login.php` — Livewire component with typed `$login_error` property (`null` | `'rate_limited'` | `'invalid'`), `$remember` bool for the remember-me toggle (defaults to `false`, opt-in), validates email (`email:rfc`, aligned with Register) and password with custom localized `messages()`, resets `$login_error` to `null` before each submission, uses a `match` expression on `LoginResult` enum cases, calls `LoginUserAction`, redirects on success, resets password and sets error state on failure. On success it sets the app locale from the signed-in user (`app()->setLocale(auth()->user()->locale)`) because the user's language is not yet applied (guest page has no HasUser), flashes a `You're signed in` toast into the session before redirecting; `mount()` re-dispatches any flashed session toast as a browser `toast` event so the toast survives the redirect.
...
- `resources/views/pages/⚡profile/profile.php` — Livewire page state (`HasUser` trait), authenticated user lookup, profile validation with localized custom `messages()`, action call, result handling, form reset or cancel behavior. The country list is resolved per application locale inside `cache()->rememberForever('countries-list-{locale}')` and ICU-collated (`Collator('fa_IR')` for fa, natural `asort` otherwise). A successful update closes the edit modal via a `close-modal` event, raises a `Profile updated` toast, and dispatches a `profile-updated` browser event whose payload now includes `initials: $this->user->initials()` so the header does not recompute initials in JS; there is no inline success alert. A `$locale` property (validated `Rule::in(['fa','en'])`) is sent to the action; when the theme **or** locale changed, `app()->setLocale($this->user->locale)` is set and the page re-lands on `/profile` (`navigate: false`) so the new language/theme applies. `deleteAccount()` resets the locale to `config('app.locale')` before redirecting home.
...
- `resources/views/pages/⚡profile/profile.blade.php` — profile display UI (avatar initials via `User::initials()`), edit modal, form fields, field errors, profile-level rate-limit and duplicate-username alerts (all labels localized through `__()`). The personal-information panel shows firstname/lastname/birthday/gender/country; a separate "Preferences" section shows Theme and Language (the saved `locale` as `فارسی`/`English`). The edit modal adds a `Language` select (`wire:model="locale"`, `leftIcon="Language"`, options `fa`/`en`) beside the `Theme` select (driven by `config('themes.name')`); the `Gender` select is still present. Birthday renders as Jalali (`Jalali::format(..., 'yyyy/MM/dd')`) when the app locale is fa and Gregorian `Y-m-d` otherwise. Datepicker years range is `[-100, 0]`.
...
- `app/Actions/Profile/UpdateProfileAction.php` — profile update business action; handles rate limiting, username uniqueness checks after the limiter gate, profile persistence (including the `locale` field), race-condition duplicate handling, and logging.
...
- Livewire validation handles required username, username format, text lengths, enum-backed gender values, valid country codes, a `locale` restricted to `['fa','en']`, a theme validated against `config('themes.name')`, and non-future birth dates.
- The chosen language (`users.locale`) is applied app-wide by the shared `HasUser::boot()` on every Livewire request; when it changes, the profile redirects without `navigate` so the whole page re-renders under the new language.
...
**Acceptance Result:** UC-03 is accepted. Authenticated users can view and edit profile information (now including a **language** preference persisted to `users.locale` that re-renders the app in the chosen language via `HasUser::boot()`), invalid input is rejected, duplicate usernames are handled cleanly, rate limiting is enforced, and the use case is covered by passing tests: 23 co-located executions (99 assertions, 20 test functions including the 4-case username-format dataset — shared with UC-04's delete-account flow), 10 action tests (25 assertions), 2 access tests, and 4 `initials()` unit tests.
