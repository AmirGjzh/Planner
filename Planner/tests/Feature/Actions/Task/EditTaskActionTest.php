...
it('updates a task successfully', function () {
    ...
    $result = app(EditTaskAction::class)->execute(
        $user, $task->id, 'Updated', 'New desc', '2026-06-15', 60, TaskPriority::High, 2, $category->id, null, editTaskRequest(),
    );
...
it('updates a task with a plan assigned', function () {
    ...
    $result = app(EditTaskAction::class)->execute(
        $user, $task->id, 'Original', null, '2026-06-01', 30, TaskPriority::Medium, 0, $category->id, $plan->id, editTaskRequest(),
    );
...
it('prevents editing a task that belongs to another user', function () {
    ...
    app(EditTaskAction::class)->execute(
        $user2, $task->id, 'Hacked', null, '2026-06-01', 30, TaskPriority::Medium, 0, $category->id, null, editTaskRequest(),
    );
});
...
it('returns rate limited after repeated edit attempts', function () {
    ...
    foreach (range(1, 5) as $attempt) {
        app(EditTaskAction::class)->execute(
            $user, $task->id, 'Attempt '.$attempt, null, '2026-06-01', 30, TaskPriority::Medium, 0, $category->id, null, editTaskRequest(),
        );
    }

    $result = app(EditTaskAction::class)->execute(
        $user, $task->id, 'Blocked', null, '2026-06-01', 30, TaskPriority::Medium, 0, $category->id, null, editTaskRequest(),
    );
...
it('allows editing again after one minute', function () {
    ...
    $result = app(EditTaskAction::class)->execute(
        $user, $task->id, 'After cooldown', null, '2026-06-01', 30, TaskPriority::Medium, 0, $category->id, null, editTaskRequest(),
    );
...
it('returns invalid category on edit', function () {
    ...
    $result = app(EditTaskAction::class)->execute(
        $user, $task->id, 'Test', null, '2026-06-01', 30, TaskPriority::Medium, 0, 999, null, editTaskRequest(),
    );
...
it('returns invalid plan on edit', function () {
    ...
    $result = app(EditTaskAction::class)->execute(
        $user, $task->id, 'Test', null, '2026-06-01', 30, TaskPriority::Medium, 0, $category->id, 999, editTaskRequest(),
    );
...
it('logs edit events', function () {
    ...
    app(EditTaskAction::class)->execute(
        $user, $task->id, 'Renamed', null, '2026-06-01', 30, TaskPriority::Medium, 0, $category->id, null, editTaskRequest(),
    );
