...
`skills/main-project/` is the skill; subfolders:

| Folder | Contents |
| --- | --- |
| `phases/Phase-0..4.md` | Vision/scope, requirements, design, data model, implementation summary |
| `docker/overview.md` | Shared docker conventions (healthchecks, env contract, wrappers) |
| `docker/development.md` | Dev workspace stack + runbook |
| `docker/production.md` | Prod nginx/php-fpm/mysql/redis stack + runbook |
| `git-setup/setup.md` | Two-remote (public `origin` / private `private`) workflow |

`.ai/` is the committed, authoritative docs. `.agents/skills/main-project` is a symlink that mirrors it automatically — edit `.ai/` only.
