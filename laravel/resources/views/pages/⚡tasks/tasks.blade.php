<div class="flex flex-1 flex-col px-4 pt-4 pb-6 sm:px-8 md:px-16">
    <x-mine.animate class="mb-6">
        <h1 class="mine-text-primary text-base font-bold">{{ __('My tasks') }}</h1>
    </x-mine.animate>
    @island(name: 'tasks-content', always: true)
        <x-mine.animate delay="50" class="mb-6 flex flex-col gap-2 sm:gap-4">
            <div class="flex w-full items-center justify-between gap-2 sm:gap-4">
                    ...
                    >
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <p class="hidden text-sm font-medium sm:inline">
                                ...
                                ><span class="hidden md:inline">{{ __('New task') }}</span>
                            </p>
                        </div>
                    </x-mine.button>
                ...
                <x-mine.dropdown group="task-filter" class="hidden w-full md:flex md:w-auto">
            ...
            <div class="flex w-full justify-between gap-2 sm:gap-4 md:w-auto">
                <x-mine.dropdown group="task-filter" class="w-full md:hidden md:w-auto">
                    <x-mine.dropdown.trigger as="div" class="w-full md:w-auto">
                        <div @class([
                            'sm:pr-4!' => app()->isLocale('en'),
                            'sm:pl-4!' => app()->isLocale('fa'),
                    ...
                    <x-mine.dropdown.content
                        class="mt-1!"
                        placement="bottom-{{ app()->isLocale('fa') ? 'end' : 'start' }}"
                    >
                        @foreach ([
                            'all' => __('All'),
                            'active' => __('Active'),
                ...
                >
                    <x-mine.dropdown.trigger as="div" class="w-full">
                        <div @class([
                            ...
                            @if (count($category_filter) > 0)
                                <span class="mine-badge-primary rounded-full px-1.5 pt-1 pb-0.5 text-xs font-semibold">
                    ...
                    <x-mine.dropdown.content
                        class="mine-scrollbar mt-1! max-h-60 overflow-y-auto"
                        placement="bottom-{{ app()->isLocale('fa') ? 'end' : 'start' }}"
                    >
                ...
                <x-mine.dropdown group="task-filter" class="w-full md:hidden">
                    <x-mine.dropdown.trigger as="div" class="w-full">
                        <div @class([
                            'sm:pr-4!' => app()->isLocale('en'),
                            'sm:pl-4!' => app()->isLocale('fa'),
                                        ...
                                        <div class="mb-4 flex gap-2">
                                            ...
                                            <div @class([
                                                ...
                                                <p @class([
                                                    ...
                                                    {{ app()->isLocale('fa') ? App\Support\PersianNumber::convert($task->estimated_minutes) : $task->estimated_minutes }} {{ __('min') }}
                                                </p>
                                            </div>
                                            <div @class([
                                                'pl-3 pr-4' => app()->isLocale('en'),
        ...
        <x-mine.animate class="mt-6 w-full"> {{ $this->tasks->links(data: ['scrollTo' => false]) }} </x-mine.animate>
    @endisland

    <x-mine.modal id="add-task-form" :close-by-clicking-away="false" :close-by-escaping="false" width="lg">
        <div class="px-6 py-6 sm:px-8">
            <div class="mb-6 flex items-center justify-between">
                <h2 class="mine-text-primary text-base font-semibold">{{ __('Add new task') }}</h2>
                <button
                    type="button"
                    @click="
                        close();
            ...
            @if ($add_error === 'rate_limited')
                <div class="mb-4">
                    <x-mine.alert
                        variant="warning"
                        title="{{ __('Too many create-task attempts!') }}"
                    >
            ...
            @if ($add_error === 'invalid_category')
                <div class="mb-4">
                    ...
                    >
                        {{ __('Please choose a valid category.') }}</x-mine.alert>
                </div>
            @endif
            ...
            @if ($add_error === 'invalid_plan')
                <div class="mb-4">
                    <x-mine.alert
                        variant="danger"
                        title="{{ __('Invalid plan!') }}"
                    >
                        {{ __('Please choose a valid plan.') }}</x-mine.alert>
                </div>
            @endif
            ...
            <form class="flex flex-col gap-4" wire:submit="addTask">
                <div>
                    <x-mine.input
                        label="{{ __('Task title') }}"
                        wire:model="add_title"
                        placeholder="{{ __('Your task title') }}"
                        leftIcon="Clipboard"
                    />
                </div>
                <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <x-mine.input
                        ...
                        wire:model="add_estimated_minutes"
                        type="number"
                        placeholder="{{ __('e.g. 30') }}"
                    />
                    <x-mine.input
                        leftIcon="Bell"
                        label="{{ __('Days before alarm') }}"
                        wire:model="add_alarm_days"
                        type="number"
    ...
    <x-mine.modal id="edit-task-form" :close-by-clicking-away="false" :close-by-escaping="false" width="lg">
        <div class="px-6 py-6 sm:px-8">
            <div class="mb-6 flex items-center justify-between">
                <h2 class="mine-text-primary text-base font-semibold">{{ __('Edit task') }}</h2>
                <button
                    type="button"
                    @click="
                        close();
            ...
            @if ($edit_error === 'rate_limited')
                <div class="mb-4">
                    <x-mine.alert
                        variant="warning"
                        title="{{ __('Too many edit-task attempts!') }}"
                    >
                        {{ __('Try again in a minute.') }}</x-mine.alert>
                </div>
            @endif
            ...
            <form class="flex flex-col gap-4" wire:submit="editTask">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <x-mine.input
                        leftIcon="Clock"
                        label="{{ __('Estimated minutes') }}"
                        wire:model="edit_estimated_minutes"
                        type="number"
                        placeholder="{{ __('e.g. 30') }}"
                    />
                    <x-mine.input
    ...
    <x-mine.modal id="delete-task-confirmation" :close-by-clicking-away="false" :close-by-escaping="false" width="lg">
        <div class="px-6 py-6 sm:px-8">
            <div class="mb-6 flex items-center justify-between">
                <h2 class="mine-text-primary text-base font-semibold">{{ __('Delete task') }}</h2>
                <button
                    type="button"
                    @click="
                        close();
    ...
    <x-mine.modal id="complete-task-confirmation" :close-by-clicking-away="false" :close-by-escaping="false" width="lg">
        <div class="px-6 py-6 sm:px-8">
            <div class="mb-6 flex items-center justify-between">
                <h2 class="mine-text-primary text-base font-semibold">{{ __('Complete task') }}</h2>
                <button
                    type="button"
                    @click="
                        close();
            ...
            @if ($complete_error === 'rate_limited')
                <div class="mb-4">
                    <x-mine.alert
                        variant="warning"
                        title="{{ __('Too many complete-task attempts!') }}"
                    >
                        {{ __('Try again in a minute.') }}</x-mine.alert>
                </div>
            @else
                <div class="mb-4">
                    <x-mine.alert
                        ...
                        title="{{ __('Are you sure?') }}"
                    >
                        {{ __('This will mark the task as completed.') }}</x-mine.alert>
                </div>
            @endif
    ...
    <x-mine.modal id="reopen-task-confirmation" :close-by-clicking-away="false" :close-by-escaping="false" width="lg">
        <div class="px-6 py-6 sm:px-8">
            <div class="mb-6 flex items-center justify-between">
                <h2 class="mine-text-primary text-base font-semibold">{{ __('Re active task') }}</h2>
                <button
                    type="button"
                    @click="
                        close();
            ...
            @if ($reopen_error === 'rate_limited')
                <div class="mb-4">
                    <x-mine.alert
                        variant="warning"
                        title="{{ __('Too many re-active-task attempts!') }}"
                    >
                        {{ __('Try again in a minute.') }}</x-mine.alert>
                </div>
            @else
                <div class="mb-4">
                    <x-mine.alert
                        ...
                        title="{{ __('Are you sure?') }}"
                    >
                        {{ __('This task will be moved back to active.') }}</x-mine.alert>
                </div>
            @endif
