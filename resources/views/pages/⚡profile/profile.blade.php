            ...
            <div class="flex items-center gap-4 md:flex-col">
                <div class="md:mt-20 size-20 shrink-0 rounded-full mine-badge-primary flex justify-center items-center">
                    <h1 class="font-bold text-xl">
                        {{ strtoupper($this->user->firstname && $this->user->lastname ? substr($this->user->firstname, 0, 1) . substr($this->user->lastname, 0, 1) : substr($this->user->username, 0, 2)) }}
                    </h1>
                </div>
                <div class="md:text-center md:flex-0 sm:flex-1 md:my-2 min-w-0">
                    <h1 class="font-bold text-md mine-text-primary mb-1 truncate">{{ $this->user->username }}</h1>
        ...
        <div class="md:flex-2 flex flex-col">
            <div class="mine-card p-6 sm:p-8 flex flex-col gap-4">
                ...
                @foreach ([
                        ...
                        'Gender' => $this->user->gender ? ucfirst($this->user->gender->value) : null,
                        'Country' => $this->countries[$this->user->country] ?? null,
                    ] as $label => $value)
        ...
        <div class="px-6 sm:px-8 py-6">
            ...
            <form class="flex flex-col gap-4" wire:submit="editProfile">
                ...
                <div class="flex flex-col sm:flex-row gap-4">
                    ...
                    <x-mine.select wire:model="country" label="Country" placeholder="Select Country" searchable>
    ...
    <x-mine.modal id="delete-account-confirmation" :close-by-clicking-away="false" :close-by-escaping="false"
        ...
        <div class="px-6 sm:px-8 py-6">
            ...
            <form wire:submit="deleteAccount" class="flex flex-col gap-6">
                <x-mine.input wire:model="password" label="Password" placeholder="Enter your password" type="password"
                    leftIcon="lock-closed" x-ref="deletePassword"
                    @open-modal.window="$nextTick(() => { if ($event.detail?.id === 'delete-account-confirmation') $refs.deletePassword?.focus(); })" />
                <div class="flex gap-4 justify-between items-center">
                    <x-mine.button type="button" @click="close(); $wire.cancelDelete()" class="mine-btn-ghost">Cancel
                    </x-mine.button>
                    <x-mine.button wire:target="deleteAccount" wire:loading:disabled class="mine-btn-danger">Delete Account
                    </x-mine.button>
                </div>
            </form>
        </div>
